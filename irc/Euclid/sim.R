rrom <- function(n, pc, tc = 0, tv = 1, cc = 0, cv = 2) {
    ## Input: n = number of variates to produce pc = probability of a contaminant tc =
    ## true distribution centre tv = true distribution variance cc = contaminating
    ## distribution center cv = contaminating distribution variance Output: rs =
    ## variates from contaminated distribution
    k <- rbinom(n, 1, 1 - pc)
    return(c(rnorm(n - k, tc, tv), rnorm(k, cc, cv)))
}

# NOTE: A general Fixed Outlier Model function provided for two normal
# distributions
rfom <- function(n, nc, tc = 0, tv = 1, cc = 0, cv = 2) {
    ## Input: n = number of variates to produce nc = number of contaminants tc = true
    ## distribution centre tv = true distribution variance cc = contaminating
    ## distribution center cv = contaminating distribution variance Output: rs =
    ## variates from contaminated distribution
    return(unlist(lapply(c(rep(1, n - nc), rep(0, nc)), function(x) {
        ifelse(x, rnorm(1, tc, tv), rnorm(1, cc, cv))
    })))
}

# NOTE: For simplicity, general standardized laplace and logistic distributions
# are provided rslogis<-function(n,l=0,s=1){ Input: n = number of variates l =
# location s = scale
# return((rlogis(n,l,s)-l)/(3.14159265358979323846/(sqrt(3)*s))) }
# rslaplace<-function(n,l=0,s=1){ Input: n = number of variates l = location s =
# scale return((rlaplace(n,l,s)-l)/(sqrt(2)*s)) }

rslogis <- function(n = n, l = 0, s = 1, lb = 0, ub = 1) {
    ## Input: n = number of variates l = location s = scale lb = lower bound of
    ## Uniform ub = upper bound of Uniform
    X <- runif(n, lb, ub)
    Y <- log(X/(1 - X))
    return((Y - l)/(3.14159265358979/(sqrt(3) * s)))
}


rslaplace <- function(n = n, l = 0, s = 1, ex.rate1 = 1, ex.rate2 = 1) {
    ## Input: n = number of variates l = location s = scale ex.rate1 = rate 1 of
    ## exponential distribution ex.rate2 = rate 2 of exponential distribution
    X <- rexp(n, ex.rate1)
    Y <- rexp(n, ex.rate2)
    W <- X - Y
    return((W - l)/(sqrt(2) * s))
...
utst.sta <- function(X, Y, md = 0) {
    # Unequal Variance Two Sample T Test Statistic A function that computes the two
    # sample t-test statistic under null hypothesis of default mean difference 0
    t <- mean(X) - mean(Y)
    t <- t - md
...
var.pool <- function(X, Y) {
    return((var(X) * (length(X) - 1) + var(Y) * (length(Y) - 1))/(length(X) + length(Y) - 
        2))
    # varsp <- ((length(X)-1)/(length(X)+length(Y)-2))*var(X); varsp <- varsp +
    # ((length(Y)-1)/(length(X)+length(Y)-2))*var(Y); return(varsp)
}

etst.sta <- function(X, Y, md = 0) {
    # Equal Variance Two Sample T Test Statistic Calculation
    varsp <- var.pool(X, Y)
...
evft.sta <- function(X, Y) {
    # F Test for testing equality of variance
    return(var(X)/var(Y))
}

tconf.uneq <- function(X, Y, dfm = df.min, cl = 0.95) {
    # Computes T-confidence interval when unequal variances
    mdiff = mean(X) - mean(Y)
...
# dfile <- file('SimulationData.csv', 'w') ftestconflvls <- c(0.01,0.05,0.1)
# function(n,pc,tc=0,tv=1,cc=0,cv=2) function(n,nc,tc=0,tv=1,cc=0,cv=2)
# function(n,l=0,s=1)
        ...
        sapply(modelsy[1], function(mody) {
            sapply(sampsizey[1], function(ssy) {
                ...
                sapply(sampsizex[1], function(ssx) {
                  X <- switch(modx, snorm = rnorm(ssx, 0, 1), rom2 = rrom(ssx, 0.2, 
                    0, 1, 0, 4), fom2 = rfom(ssx, 2, 0, 1, 0, 4), logist = rslogis(ssx, 
